...
function AuthComponent(props) {
    ...
    const getLoginInfo = useCallback(
        (ctx) => withPromiseAndDispatch(getLoginInfoAsync, ctx, dispatch),
        [dispatch]
    );
    ...
    const bootstrapAsync = async () => {
        try {
            ...
            // check here ! optimize here
            if (!isPermissionAccess) {
                navigate("/"); // redirect: public router /
            }
        } catch (err) {
    ...
    return (
        <>
            {isAuthen ? (
                <React.Suspense fallback={<div className="loading" />}>{children}</React.Suspense>
            ) : (
